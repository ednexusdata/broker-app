...
public class ImportRequestMappingsJob : IJob
{
    ...
    public async Task ProcessAsync(Job jobInstance)
    {
        ...
        foreach(var mapping in mappings.Where(x => x.PayloadContentAction?.Process == true).ToList())
        {
            await _jobStatusService.UpdatePayloadContentActionStatus(jobInstance, mapping.PayloadContentAction!, Core.PayloadContentActionStatus.Imported, "Finished importing {0}.", mapping.MappingType);
        }
